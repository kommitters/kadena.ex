defmodule Kadena.Types.CommandPayload do
  ...
  """
  alias Kadena.Types.{ChainwebNetworkID, MetaData, PactPayload, Signer, SignerList}

  @behaviour Kadena.Types.Spec

  @type network_id :: ChainwebNetworkID.t() | nil
  @type payload :: PactPayload.t()
  @type signers :: SignerList.t()
  ...
  def new(args) do
    ...
    {:ok, nonce} <- validate_nonce(nonce) do
      %__MODULE__{network_id: network_id, payload: payload, signers: signers, meta: meta, nonce: nonce}
    end
  end

  @spec validate_network_id(network_id :: atom()) :: validation()
  defp validate_network_id(network_id) do
    case ChainwebNetworkID.new(network_id) do
      %ChainwebNetworkID{} = network_id -> {:ok, network_id}
  ...
  defp validate_signers(signers) do
    case SignerList.new(signers) do
      %SignerList{} = signers -> {:ok, signers}

defmodule Kadena.Types.PactExec do
  ...
  defp validate_pact_id(_pact_id), do: {:error, :invalid_pact_id}

  @spec validate_step(step :: step()) :: validation()
  defp validate_step(%Step{} = step), do: {:ok, step}
  defp validate_step(_step), do: {:error, :invalid_step}
  ...
  defp validate_step_count(_step_count), do: {:error, :invalid_step_count}
  ...
  defp validate_executed(_executed), do: {:error, :invalid_executed}
  ...
  defp validate_step_has_rollback(_step_has_rollback), do: {:error, :invalid_step_has_rollback}
  ...
  defp validate_yield(_yield), do: {:error, :invalid_yield}
end
